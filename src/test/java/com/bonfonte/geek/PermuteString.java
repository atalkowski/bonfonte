...
public class PermuteString {
  // A similar class could be "Permute<T> where a List<T> is the collection to be permuted.
  ...
  public String next() {
    while (true) {
      Pair pair = pop();
      if (pair == null) throw new RuntimeException("No next item available");
      switch (pair.suffix.length()) {
        ...
        default:
          ...
          // That suffix = "bcd" generates 3 char/complements: ("b","cd"), ("c", "bd") and ("d", "bc");
          // So the one pair ("a", "bcd") will be popped off the stack
          // and will replace itself with its 3 new pairs ("ab", "cd"), ("ac", "bd") and ("ad", "bc")
          ...
          // Once a suffix is one character long we know there are no further permutations of the Pair.
          ...
          int inx = length - 1;
          // We push pairs in reverse order so that when popped and output by a next() call they are in nice order.
          // E.g for s = "abdcef" the first permutation output will be "abcdef" ... and the last "fedcba"
          while (inx >= 0) {
